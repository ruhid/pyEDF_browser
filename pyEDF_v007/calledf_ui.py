...
class MyForm(QMainWindow):
    ...
    def update_rt_plot(self):
        ...
        # ad text
        text = pg.TextItem(
            html=f'<div style="text-align: center"><span style="color: #FFF;font-size: 16pt;">{self.current_ch} {self.current_color}</span></div>',
            anchor=(0.5, 0.5))
        self.p2.addItem(text)

    def rt_trshold_onset_detect(self):
        reshapY=self.ui_edf.spinBoxReshapeY.value()
        reshapX=self.ui_edf.spinBoxReshapeX.value()
        mov_avg_wind=self.ui_edf.spinBoxMwAvgWind.value()
        window_prct=self.ui_edf.doubleSpinBoxWindPrct.value()
        pre_rt_start=self.ui_edf.spinBoxAnzRTStart.value()
        pre_rt_end=self.ui_edf.spinBoxAnzRtEnd.value()
        times_std=self.ui_edf.spinBoxTimesStd.value()

        signal=self.output["y"]
        signalx=self.output["x"]
        treshold=std_trshld(signal,
                            self.sample_rate,
                            pre_rt_time=[pre_rt_start,pre_rt_end],
                            times_std=times_std
                            )["treshold"]

        detect_start=self.ui_edf.spinBoxTrsDctStart.value()
        onset=detect_trsh_rt(signal=signal,
                             treshold=treshold,
                             reshape=(reshapY,reshapX),
                             mov_avg_window=mov_avg_wind,
                             mov_avg_window_prcnt=window_prct)

        onset=onset/self.sample_rate
...
def detect_trsh_rt(signal, treshold, reshape, mov_avg_window=10, mov_avg_window_prcnt=0.3):
    ...
    signal_resample = signal.reshape(reshape).mean(axis=1)
