...
class MyForm(QMainWindow):
    ...
    def load_rts_csv(self):
        ...
        try:
            self.RT_fileNameCSV, OK = QFileDialog.getOpenFileName(self, 'Open file', "", "EDF files (*.csv)",options=self.options)
            if OK:
    ...
    def read_original(self):
        ...
        self.rts_df = pd.DataFrame(data, columns=["trace", "rt", "color", "button", "fsr"])
    ...
    def save_rt(self):
        ...
        QTimer.singleShot(100,
                          lambda: self.ui_edf.tableViewRTS_plot_window.scrollTo(index, QAbstractItemView.PositionAtCenter))
